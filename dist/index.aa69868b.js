...
/* Закрыть модальное окно нажатием на пустой экран, для кнопок:
    ...
    "расчитать стоимость" */ window.onclick = function(event) {
...
if (popapPriceBtns.length > 0) for(let i = 0; i < popapPriceBtns.length; i++){
...
tabsTriggersItem.forEach((item)=>item.addEventListener("click", function(e) {
        e.preventDefault();
        const id = e.target.getAttribute("href").replace("#", "");
        tabsTriggersItem.forEach((child)=>child.classList.remove("tabs-triggers__item_active"));
        tabsContentItem.forEach((child)=>child.classList.remove("tabs-content__item_active"));
...
// const popapStyleWrapp = document.querySelector('.popap-style__wrapper');
// Открыть модальное окно и поиск всех кнопок для открытия!
if (tabsContentBtns.length > 0) for(let i1 = 0; i1 < tabsContentBtns.length; i1++){
    let popapStyleBtn = tabsContentBtns[i1];
...
// const popapRecordsWrapp = document.querySelector('.popap-records__wrapper');
// Открыть модальное окно и поиск всех кнопок для открытия!
if (popapRecordsBtns.length > 0) for(let i2 = 0; i2 < popapRecordsBtns.length; i2++){
    let popapRecordsBtn = popapRecordsBtns[i2];
